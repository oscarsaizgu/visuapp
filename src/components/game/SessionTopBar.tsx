...
interface Props { hechas: number; total: number; combo: number; /** Veloz: en lugar de progreso, los aciertos. */ aciertos?: number }

export function SessionTopBar({ hechas, total, combo, aciertos }: Props) {
  return (
    <header className={styles.bar}>
      ...
      {aciertos !== undefined ? (
        <p className={styles.score} aria-live="polite"><strong>{aciertos}</strong> {aciertos === 1 ? 'acierto' : 'aciertos'}</p>
      ) : (
